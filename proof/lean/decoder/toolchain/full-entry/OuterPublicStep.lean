...
/-- Public decode of the original ADD encoding followed by the unchanged
    production execution theorem. The actual initial configuration disables
    MOP. Memory-read contracts and the existing Sail readiness premises remain
    explicit; physical memory coupling and reset reachability are not claimed. -/
theorem cold_public_add_step {M R : Type} (mi : ckb_vm.memory.Memory M R)
    ...
    ∃ inst decoder' m' s',
      ckb_vm.decoder.DefaultDecoder.Insts.Ckb_vmDecoderInstDecoder.decode mi (initial 2#u32) mem (view m).pc =
  ...
  have hd := decode_cold_public_word mi mem memAfterFetch (view m).pc size hsize hbound w hraw rustFetch
...
#print axioms cold_public_add_step
end
end OuterAdd
