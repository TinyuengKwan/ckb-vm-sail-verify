...
class PublicCleanTests(unittest.TestCase):
    def setUp(self):
        ...
        self.inputs = public.gate.ROOT / 'artifacts/decoder-inputs/rebuilt-v2'
        stack = ExitStack()
        ...
        stack.enter_context(patch.object(locations, 'load', return_value={
            'directory': str(self.inputs), 'payload': str(self.inputs / 'payload')}))
    ...
    def test_source_only_reaches_no_cache_build(self):
        with self.assertRaises(ReachedBuild):
            public.build(self.out, self.run_stage, self.report, self.inputs)
    ...
    def test_invalid_installed_inputs_stop_before_source_setup(self):
        with patch.object(locations, 'load', side_effect=RuntimeError('bad installed inputs')):
            with self.assertRaisesRegex(RuntimeError, 'bad installed inputs'):
                public.build(self.out, self.run_stage, self.report, self.inputs)
    ...
    def test_main_audit_receives_clean_import_path(self):
        ...
        def run(name, command, cwd, env, **kwargs):
            ...
            raise ReachedBuild()
        with self.assertRaises(ReachedBuild):
            public.build(self.out, run, self.report, self.inputs)
